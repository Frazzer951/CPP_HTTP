...
namespace http
{
  ...
  inline namespace detail
  {
    ...
    public:
      ...
      void connect( const struct sockaddr * address, const socklen_t addressSize, const std::int64_t timeout )
      {
        ...
        while( result == -1 && WSAGetLastError() == WSAEINTR ) result = ::connect( endpoint, address, addressSize );
...
#else
        auto result = ::connect( endpoint, address, addressSize );
        while( result == -1 && errno == EINTR ) result = ::connect( endpoint, address, addressSize );
    ...
    private:
      ...
      void select( const SelectType type, const std::int64_t timeout )
      {
        ...
        timeval selectTimeout { static_cast<decltype( timeval::tv_sec )>( timeout / 1000 ),
  ...
  public:
                   ...
                   const std::chrono::milliseconds  timeout = std::chrono::milliseconds { -1 } )
    {
      ...
      enum class State { statusLine, headers, body } state = State::statusLine;
        ...
        if( state != State::body )
          for( ;; )
          {
            ...
            else if( state == State::headers )    // RFC 7230, 3.2. Header Fields
            {
              ...
              if( loumnPosition == std::string::npos ) throw ResponseError( "Invalid header: " + line );
              ...
              auto isNotWhiteSpace = []( char c ) { return c != ' ' && c != '\t'; };
              ...
              // rtrim
              headerValue.erase( std::find_if( headerValue.rbegin(), headerValue.rend(), isNotWhiteSpace ).base(),
                                 headerValue.end() );
              ...
              {
                if( headerValue == "chunked" ) chunkedResponse = true;
                else
